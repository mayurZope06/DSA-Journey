...
public:
    ...
    int solveTD(string &word1, string &word2, int i, int j, vector<vector<int>> &dp)
    {
        ...
        int ans = 0;
        if (word1[i] == word2[j])
            ans = solveTD(word1, word2, i + 1, j + 1, dp);
        else
        {
            // If current characters don't match, we have three options:
            int insertOp = solveTD(word1, word2, i, j + 1, dp);      // Insert a character into word1
            int removeOp = solveTD(word1, word2, i + 1, j, dp);      // Remove a character from word1
            int replaceOp = solveTD(word1, word2, i + 1, j + 1, dp); // Replace the character in word1
    ...
    int solveBU(string &word1, string &word2)
    {
        ...
        for (int i = word1.size() - 1; i >= 0; i--)
        {
            for (int j = word2.size() - 1; j >= 0; j--)
            {
                ...
                {
                    // If current characters don't match, we have three options:
                    int insertOp = dp[i][j + 1];      // Insert a character into word1
                    int removeOp = dp[i + 1][j];      // Remove a character from word1
                    int replaceOp = dp[i + 1][j + 1]; // Replace the character in word1
    ...
    int solveSO(string &word1, string &word2)
    {
        ...
        for (int i = word1.size() - 1; i >= 0; i--)
        {
            for (int j = word2.size() - 1; j >= 0; j--)
            {
                ...
                {
                    // If current characters don't match, we have three options:
                    int insertOp = curr[j + 1];  // Insert a character into word1
                    int removeOp = next[j];      // Remove a character from word1
                    int replaceOp = next[j + 1]; // Replace the character in word1
