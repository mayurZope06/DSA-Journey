...
class Solution
{
    public:
        void solve (string &output, unordered_map<char, string> &mapping, vector<string> &ans, string digits, int i)
        {
            ...
            // 1 case hum solve karenge, bakki recursion sambhal lega
            char digit = digits[i];
            string mappedString = mapping[digit];
            for(char ch: mappedString)
            {
                // hr character ke liye recursive call maarni hai, after including in output string
                output.push_back(ch);
                // bakki recursion dekhlega
